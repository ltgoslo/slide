...
DATASETS = {
    ...
    "ud_plus": "../code/train-multi.jsonl.gz",
    "multilingual_tatoeba": "../data/multilingual_multilabel_tatoeba_sentences_v2.jsonl.gz",
    "multilingual_ud": "../data/multilingual_multilabel_ud_sentences_v2.jsonl.gz"
}
...
def train_sub_epoch(model, train_loader, val_loader, optimizer, lr_scheduler, device, metrics, args, epoch: int, best_metric = 0.0):
    model.train()
    ...
    for i, batch in enumerate(progress_bar):
        ...
        if i % 100 == 0:
            ...
            if max_metric > best_metric:
                ...
                best_step = epoch * len(train_loader) + i
    ...
    return best_metric, best_step, best_model_weights
...
def train(model, args, train_loader, val_loader, int_to_label, label_vocab, device, metrics):
    ...
    for epoch in range(args.epochs):
        best_metric, best_step, best_model_weights = train_sub_epoch(model, train_loader, val_loader, optimizer, lr_scheduler, device, metrics, args, epoch, best_metric)
        calculated_metrics, val_loss = evaluate(model, val_loader, device, metrics, args)
        ...
        if max_metric > best_metric:
            best_metric = max_metric
            best_step = len(train_loader) + len(train_loader) * epoch
            best_model_weights = deepcopy(model.state_dict())
    ...
    print(f"The best metric score was obatined after {best_step} steps, with a best metric score of: {best_metric}.")
    print(f"Loading model as it was after epoch {best_step} steps.")
